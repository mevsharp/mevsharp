...
namespace MEVSharp.UI.API.Controllers
{
    ...
    public class BuilderController : ControllerBase
    {
        ...
        )
        {
            ...
            {
                controllerLogger.LogDebug("qqqqqqqqqqqqqqqqq");
                return BadRequest();
            }
        }
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (var item in result.OrderByDescending(x => x.Result.Value))
                {
                    ...
                    {
                        controllerLogger.LogInformation(
                            $"Found and verified winning bid for {item.Result.BlockHash} worth {item.Result.Value} ETH on relay {item.Client.Resource.Client.BaseAddress}"
                        );
                        return Ok(item.Result.DTO);
                    }
                }
                controllerLogger.LogError(
                    $"Failed to retrieve a verifiable header for all relays with parent_hash: {parent_hash} and pubkey: {pubkey}"
                );
            ...
            {
                controllerLogger.LogError($"ParentHashMismatchException: {e}");
        ...
        public async Task<IActionResult> GetPayload([FromBody] BlindedBlockRequest request)
        {
            ...
            long msIntoSlot =
                DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - (slotStartTimestamp * 1000);
            controllerLogger.LogInformation(
                $"submitBlindedBlocks started {msIntoSlot} milliseconds into slot {slot} with blockHash: {blockHash} and parentHash: {parentHash}"
            );
            controllerLogger.LogDebug(
            ...
            try
            {
                ...
                foreach (var response in responses)
                {
                    ...
                    {
                        controllerLogger.LogDebug(
                            $"Block hash matched: {request.Message.Body.ExecutionPayloadHeader.BlockHash} == {calculate.CalculatedHash}, forwarding to validator"
                        );
                    ...
                    {
                        controllerLogger.LogError(
                            $"Block hash mismatch: {request.Message.Body.ExecutionPayloadHeader.BlockHash} != {calculate.CalculatedHash}"
                        );
                    }
                }
            ...
            {
                controllerLogger.LogError(e.Message);
                return StatusCode((int)HttpStatusCode.NotAcceptable, e.Message);
            }
        }
    }
